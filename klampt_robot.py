...
class Robot:
    def __init__(self, name, active_joint_idx):
        ...
        self._world.loadElement(f"{robot_dir_path}/{name}.rob") # pr2, atlas, baxter, robonaut2
        ...
        self.dof = len(active_joint_idx)
        self._klampt_ee_link = self._robot.link(active_joint_idx[-1]) # end effector link
    ...
    def _driver_vec_from_x(self, x: np.ndarray):
        ...
        """
        assert x.size == self.dof, f"x doesn't have {self.dof} (n_dofs) elements ({self.dof} != {x.size})"
        assert x.shape == (self.dof,), f"x.shape must be (n_dofs,) - ({(self.dof,)}) != {x.shape}"
        x = x.tolist()
    ...
    def _x_to_qs(self, x: np.ndarray):
        ...
        """
        assert x.ndim == 2 and x.shape[1] == self.dof, f"x.shape: {x.shape}, dof: {self.dof}"
        n = x.shape[0]
        qs = []
        for i in range(n):
    ...
    def _Q_to_Qvec(self, Q: np.ndarray):
        assert Q.ndim == 2 and Q.shape[1] == self.dof, f"Q.shape: {Q.shape}, dof: {self.dof}"
        Q_vec = np.zeros((Q.shape[0], len(self._robot.getConfig())))
        Q_vec[:, self._active_joint_idx] = Q
        return Q_vec
    
    def _Q_from_Qvec(self, Qvec: np.ndarray):
        assert Qvec.ndim == 2 and Qvec.shape[1] == len(self._robot.getConfig()), f"Qvec.shape: {Qvec.shape}, robot_config: {len(self._robot.getConfig())}"
        Q = Qvec[:, self._active_joint_idx]
        return Q
    
    def forward_kinematics_klampt(self, Q: np.ndarray) -> np.ndarray:
    ...
    def random_sample_QP(self, n: int):
        """Randomly sample n joint angles within the joint limits"""
        Q = np.zeros((n, self.dof))
        for j in range(self.dof):
            Q[:, j] = np.random.uniform(self.active_joint_min[j], self.active_joint_max[j], n)
        
        P = self.forward_kinematics_klampt(Q)
        
        return Q, P
    
    def inverse_kinematics_klampt(self, p: np.ndarray):
        """Inverse kinematics using the klampt library"""
        assert p.ndim == 1 and p.shape[0] == 7, f"p.shape: {p.shape}"
        max_iterations = 1500
        q = np.zeros((self.dof))
        R = so3.from_quaternion(p[3:])
        t = p[:3]
    ...
    def __repr__(self):
        # print robot name, dof, and active joint names, and joint limits
        return f"Robot: {self.name}, dof: {self.dof}, active joints: {self.active_joint_names}, joint limits: {self.get_active_joint_limits()}"
...
Q, P = robot.random_sample_QP(5)

qik = robot.inverse_kinematics_klampt(P[0])
print(qik)
pik = robot.forward_kinematics_klampt(np.array([qik]))
# print Pik, p0
