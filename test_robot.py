...
class TestBaxterArm(unittest.TestCase):
    ...
    #     ans = np.array([[-0.16822442,  0.04313561,  0.05650815,  0.21466062,  0.59048799, -0.62277572, -0.46625654],[ 0.24242598,  0.24023615,  1.30156787,  0.96960093, -0.16199757, -0.06125573, -0.17285415],[ 0.44014173,  0.16105797,  1.24671986,  0.6796619 ,  0.47265209,  0.11033818,  0.54998654],[-0.11076718,  0.47755718,  1.09363983,  0.34689041,  0.3038278 ,  0.06995597, -0.88456875],[ 0.27640937,  0.82460814,  0.95220267,  0.15114884,  0.1529556 ,  0.38501375,  0.89750935]])

    #     P = self.robot.forward_kinematics(Q)

    #     np.testing.assert_allclose(P, ans, atol=1e-5)

    def test_inverse_kinematics_klampt(self):
        Q = np.array(
            [
                [
                    0.82384949,
        ...
        for i, p in enumerate(P):
            q = self.robot.inverse_kinematics_klampt(p, max_iterations=500)
            Qik[i] = q
        Pik = self.robot.forward_kinematics(Qik)
        np.testing.assert_allclose(P, Pik, atol=1e-5)
